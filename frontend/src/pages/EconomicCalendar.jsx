...
class MacroErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error('EconomicCalendar error:', error, info);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-6 text-center space-y-3">
          <p className="text-sm text-destructive font-medium">Une erreur est survenue lors du chargement de cette rubrique.</p>
          <button
